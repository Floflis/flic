...
if [[ "$diruse" != "0" ]];then
#mv "$latestbuild"/ "$year"/;
echo "New build initiated!"; #echo "Previous build has been archived under the '$year' folder.";fi # move a previous build only if its not the first build
