...
if [[ "$diruse" != "0" ]];then
#mv "$latestbuild"/ "$currentyear"/;
echo "New build initiated!"; #echo "Previous build has been archived under the '$currentyear' folder.";fi # move a previous build only if its not the first build
